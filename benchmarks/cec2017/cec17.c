...
const char * relative_path  = "/.fwa/cec2017/";
...
void eval(double *x, int nx, int mx, double *f, int func_num)
{
	...
	if (ini_flag==0)
	{
        ...
        char Path[FILENAME_MAX], dirname[FILENAME_MAX];
		const char * tmp;
        if ((tmp = getenv("HOME")) == NULL) {
            tmp = getpwuid(getuid())->pw_dir;
        }
        strcpy(dirname, tmp);
		strcat(dirname, relative_path);
        //printf(dirname);
        //printf("\n");
		...
		/* Load Matrix M*/
        strcpy(Path, dirname);
		sprintf(FileName, strcat(Path,"/input_data/M_%d_D%d.txt"), func_num, nx);
		...
		/* Load shift_data */
        strcpy(Path, dirname);
		sprintf(FileName, strcat(Path, "/input_data/shift_data_%d.txt"), func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
            printf("\n");
...
*/
	...
	for (i = 0; i < mx; i++)
	{
		...
		case 1:
			...
			break;
		case 2:
			dif_powers_func(&x[i*nx],&f[i],nx,OShift,M,1,1);
			f[i]+=200.0;
			break;
		case 3:
